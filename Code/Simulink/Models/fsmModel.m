function FR = fsmModel(X,req)
  TimeSteps = 51;  
  nbrInputs = 4;
  curPath=fileparts(which('fsm.m')); 
  addpath(curPath);

  modelpath = strrep(curPath,'Code\Simulink\Models','Benchmark\Simulink Models\1_fsm\');
  addpath(modelpath);
  disp(modelpath);
  open_system(strcat(modelpath,'bscfsm.slx'));   
  cs = getActiveConfigSet('bscfsm');
  model_cs = cs.copy;
...
% Signals initialization 
    ...
    % load data file
    save var_fsm.mat var;
    load('var_fsm.mat', 'var');

    % Solve the problem of unrecognized external input variable 'var'
    hws = get_param('bscfsm', 'modelworkspace');
    list = whos;
    N = length(list);
    for  i = 1:N
        hws.assignin(list(i).name,eval(list(i).name));
    end   
    simOut = sim('bscfsm.slx', model_cs);
