function PopObj = decisiontreeSoTA(run,element,mlmodel,initialSimNum,problem,req,evaluation)
            ...
            if H > initialSimulationNum
                ...
                %inputs
                listOfBpaths = pyrunfile("SoTA.py","listOfBpaths",data_file_location = data_file_location,verify_file_location = verify_file_location, run = string(run)); %"finalListOfRanges"
    ...
    %                 disp(templateArray{1}{2});
                ...
                for kk = 1 : length(cP)
                    ...
                    for i = 1: length(path)
                        ...
                        for ind = 1: length(inputVars)
                            if inputVars{ind} == varName
                                if lowRange ~= py.NoneType
                                    prevTemplateArray.prevTemplateArray{ind}{1} = lowRange;
                                end
                                if highRange ~= py.NoneType
                                    prevTemplateArray.prevTemplateArray{ind}{2} = highRange;
                                end
                                break
                            end
                        end
                    ...
                    for j = 1: 1
                        ...
                        if strcmp(req,"R12.1")
                            for k = 1:9                
                                ...
                                PopDec(:,k) = round((PopDec(:,k ) * (prevTemplateArray.prevTemplateArray{k}{2} - prevTemplateArray.prevTemplateArray{k}{1})) + prevTemplateArray.prevTemplateArray{k}{1});
