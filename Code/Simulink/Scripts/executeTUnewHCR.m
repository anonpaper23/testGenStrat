%Entry point function to run test generation for Tustin Simulink model
function executeTUnewHCR()
            curPath=fileparts(which('executeTUnewHCR.m')); 
...
%% TUSTIN-ars - Test generation                
            ...
            %Possible values:
            %"R1a","R1b","R1c","R1d","R1e","R2a","R2b","R4a","R4b"
            req = ["R1b"];
            ...
            for r = 1:length(runs)
                for i = 1:length(models)
                    for j = 1:length(req)
                        timeData = [];
                        startTime = tic;
                        %NOTE: To run requirement R4a and R4b, replace
                            %value of simulink model from "TU" to "TU4"
                        if strcmp(models(i),'genLog')
                            %Guided search - Logistic Regression
                            eachRun('generateLog',@ars,'R','TU',req(j),3500,runs(r), models(i),300,608);
                        elseif strcmp(models(i),'ensemble')
                            %Ensemble 
                            eachRun('ensemble',@ars,'R','TU',req(j),3500,runs(r), models(i),300,600);
                        elseif strcmp(models(i),"genRegTree")
                            %Guided search - Regression Tree
                            eachRun('generate',@ars,'R','TU',req(j),3500,runs(r), models(i),300,618);
                        elseif strcmp(models(i),"random")
                            eachRun('random',@ars,'R','TU',req(j),3500,runs(r), models(i),300,600);
                        else
                            %Individual surrogates i.e nn, svr, rf, gpr,
                            %gprnl etc.
                            eachRun('optimize_r',@ars,'R','TU',req(j),3500,runs(r), models(i),300,600);
                        end
...
%                         eachRun('verify',@ars,'R','TU4',req(j),1,runs(r), models(i),300,600);
                    end
                end
            end
end
