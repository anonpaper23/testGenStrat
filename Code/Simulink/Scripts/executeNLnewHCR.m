%Entry point function to run test generation for NLGuidance (NLG) Simulink model
function executeNLnewHCR()
            curPath=fileparts(which('executeNLnewHCR.m')); 
            ...
            fid = fopen('test_NLnewHCR.csv','wt');
            fprintf(fid,'%s\n',textHeader);            
            fclose(fid);  
                ...
                for i = 1: length(models)
                    for j = 1: length(req)
                        ...
                        if strcmp(models(i),'genLog')
                            ...
                            eachRun('generateLog',@ars,'R','NL',req(j),800,runs(r), models(i),300,539);
                        elseif strcmp(models(i),"random")
                            eachRun('random',@ars,'R','NL',req(j),800,runs(r), models(i),300,517);
                        elseif strcmp(models(i),'genRegTree')
                            %Guided search - Regression Tree
                            eachRun('generate',@ars,'R','NL',req(j),800,runs(r), models(i),300,503);
                        elseif strcmp(models(i),'ensemble')
                            %Ensemble 
                            eachRun('ensemble',@ars,'R','NL',req(j),800,runs(r), models(i),300,600);
                        else
                            %Individual surrogates i.e nn, svr, rf, gpr,
                            %gprnl etc.
                            eachRun('optimize_r',@ars,'R','NL',req(j),800,runs(r), models(i),300,600);
                        end
...
%                         eachRun('verify',@ars,'R','NL',req(j),1,runs(r), models(i),300,600);
                    end
                end
            end
